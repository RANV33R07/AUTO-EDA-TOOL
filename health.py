...
def perform_feature_scaling_and_pca_on_selected_columns(df_selected):
    ...
    unimputed_missing_df = df_selected[numerical_columns][df_selected[numerical_columns].isnull().any(axis=1)]
    st.write(unimputed_missing_df)

    # Handling missing values with imputation
    imputer = SimpleImputer(strategy='median')  # Using median instead of mean for robustness
    ...
    # Check for outliers and unrealistic values after imputation
    st.subheader("Outliers and Unrealistic Values After Imputation:")
    ...
    for column in numerical_columns:
        ...
        outliers_df[column] = df_selected[(df_selected[column] < lower_bound) | (df_selected[column] > upper_bound)][column]
